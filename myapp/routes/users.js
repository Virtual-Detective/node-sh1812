...
router.post('/addAction', function(req, res, next) {
 ...
 tel *= 1;
 age *= 1;
 sql.find('sh1812', 'users', { tel: tel }).then(data => {
	if (data.length == 0) {
		// 表示没有查询到数据 --- 可以添加该用户 -- 先加密  -- 后添加
		password = md5(password);
		sql.insert('sh1812', 'users', { tel, nickname, password, age })
			.then(() => {
				res.redirect('/users');
			})
			.catch((err) => {
...
router.get('/remove', function(req, res, next) {
	const { tel } = req.query;
  sql.remove('sh1812', 'users', { tel }).then(() => {
	  res.redirect('/users');
  }).catch((err) => {
	  res.redirect('/users');
  })
...
router.post('/updateAction', function(req, res, next) {
	...
	tel = tel * 1;
  sql.update('sh1812', 'users', 'updateOne', { tel }, {$set: { nickname }})
  .then(() => {
...
// const xlsx = "D:/前端/假期项目/myapp/stu.xlsx";//本地地址
const xlsx = "/usr/local/node-pro/test/stu.xlsx";//服务器地址
router.get('/importUsers', (req, res, next) => {
	filemd.analysisdata(xlsx).then(obj => {
		...
		// res.send(result)
		sql.insert('sh1812', 'users', result).then(() => {
			res.redirect('/users')
		})
	})
});
...
router.get('/search', (req, res, next) => {
	...
	sql.find('sh1812', 'users', { nickname: eval('/'+nickname+'/') }).then(data => {
		// res.send(data)
		sql.distinct('sh1812', 'users', 'age').then(ageArr => {
			res.render('users', {
				activeIndex: 2,
...
router.get('/ageSearch', (req, res, next) => {
	let { age } = req.query;
	age *= 1;
	sql.find('sh1812', 'users', { age }).then(data => {
		// res.send(data)
...
router.get('/sort', (req, res, next) => {
	...
	let sortData = {};
	sortData[type] = num * 1;
	sql.sort('sh1812', 'users', sortData).then(data => {
		// res.send(data)
		sql.distinct('sh1812', 'users', 'age').then(ageArr => {
			res.render('users', {
				activeIndex: 2,
