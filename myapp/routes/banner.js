...
router.get('/', function(req, res, next) {
  ...
  sql.find('sh1812', 'banner', {}).then(data => {
...
router.post('/addAction', function(req, res, next) {
  ...
  imgid *= 1;
  sql.find('sh1812', 'banner', { imgid: imgid }).then(data => {
    if (data.length == 0) {
      // 表示没有查询到数据 --- 可以添加
...
router.get('/remove', function(req, res, next) {
  ...
  console.log(imgid);
  imgid *=1;
  sql.remove('sh1812', 'banner', { imgid }).then(() => {
    res.redirect('/banner');
  }).catch((err) => {
    res.redirect('/banner');
  })
...
router.post('/updateAction', function(req, res, next) {
  let { imgid,imgurl } = req.body;
  imgid *= 1;
  sql.update('sh1812', 'banner', 'updateOne', { imgid }, {$set: { imgurl }})
    .then(() => {
...
router.get('/search', (req, res, next) => {
  ...
  imgid *=1;
  console.log(imgid);
  sql.find('sh1812', 'banner', { imgid }).then(data => {
    // res.send(data)
    res.render('banner', {
      activeIndex: 4,
      totalNumber: 1,
...
router.get('/importBanners', (req, res, next) => {
  filemd.analysisdata(xlsx).then(obj => {
    ...
    // res.send(result)
    // 将商品信息存入到数据库
    sql.insert('sh1812', 'banner', arr).then(() => {
      //商品信息存储好后，返回到商品页面
      res.redirect('/banner')
    })
  })
