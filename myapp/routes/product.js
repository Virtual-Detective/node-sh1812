...
router.post('/addAction', function(req, res, next) {
  ...
  sql.find('sh1812', 'product', { productId: productId }).then(data => {
    if (data.length == 0) {
      // 表示没有查询到数据 --- 可以添加该商品
      sql.insert('sh1812', 'product', { productId,productName,imgUrl,price,sale,color,count,brand })
        .then(() => {
  ...
  filemd.analysisdata(xlsx).then(obj => {
    ...
    data.map((item, index) => {
      if(index !== 0) {
		...
		// res.send(result)
    // 将商品信息存入到数据库
		sql.insert('sh1812', 'product', arr).then(() => {
		  //商品信息存储好后，返回到商品页面
			res.redirect('/product')
		})
	})
...
router.post('/updateAction', function(req, res, next) {
  ...
  sql.update('sh1812', 'product', 'updateOne', { productId }, {$set: { productName,imgUrl,price,sale,color,count,brand }})
    .then(() => {
...
router.get('/remove', function(req, res, next) {
  ...
  productId *=1;
  sql.remove('sh1812', 'product', { productId }).then(() => {
    res.redirect('/product');
  }).catch((err) => {
    res.redirect('/product');
  })
...
router.get('/search', (req, res, next) => {
  ...
  console.log(req.query);
  console.log(brand);
  sql.find('sh1812', 'product', { brand: eval('/'+brand+'/') }).then(data => {
    // res.send(data)
    console.log(data);
    sql.distinct('sh1812', 'product', 'brand').then(brandArr => {
      res.render('product', {
...
router.get('/brandSearch', (req, res, next) => {
  let { brand } = req.query;
  sql.find('sh1812', 'product', { brand }).then(data => {
    // res.send(data)
    sql.distinct('sh1812', 'product', 'brand').then(brandArr => {
      res.render('product', {
        activeIndex: 3,
...
router.get('/sort', (req, res, next) => {
  ...
  let sortData = {};
  sortData[type] = num * 1;
  sql.sort('sh1812', 'product', sortData).then(data => {
    // res.send(data)
    sql.distinct('sh1812', 'product', "price").then(brandArr => {
      res.render('product', {
        activeIndex: 3,
        totalNumber: 1,
        pageCode: 1,
